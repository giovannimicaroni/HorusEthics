...
if __name__ == '__main__':
    ...
    # print(data.files)
    lista_noises = []
    dir = 'outputKANFace/outputKANFace'
    i = 1
    for filename in os.listdir(dir):
        i+=1
        file_path = os.path.join(dir, filename)
        current_npz = np.load(file_path) #opens the .npz file that contains the noiseprint++
        img = Image.open('patchesKANFace/'+filename[:-4]) #open the image related do the .npz
    ...
    np.save('np++KANFace.npy', avg_noise_array)
    print('Array saved as np++KANFace.npy')