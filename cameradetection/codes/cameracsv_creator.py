...
root_directory = 'C:/Users/g2317/OneDrive/Documentos/Unicamp/IC/Horus/cameradetection/5camerasdata'
...
for subdir, _, files in os.walk(root_directory):
    for file in files:
        ...
        if ('ipad') in file_path:
            int_device = 0
        elif ('iphone') in file_path:
            int_device = 1
        elif ('s9') in file_path:
            int_device = 2
        elif ('s22') in file_path:
            int_device = 3
        elif ('tabs6') in file_path:
            int_device = 4
        
        # Append the file path and a placeholder integer (e.g., 0) to the data list
        data.append([file_path, int_device])

# Create a DataFrame from the data list
df = pd.DataFrame(data, columns=['image_path', 'camera'])

# Save the DataFrame to a CSV file
output_csv_path = '5camera.csv'
